...
test.describe("Resume Page - Full Coverage", () => {
  ...
  // About Section
  test("About section shows name, title, summary, and avatar", async ({
    page,
  }) => {
  ...
  // Experience
  test("Experience section renders experience cards with company, role, and description", async ({
    page,
  }) => {
  ...
  test("Education section renders milestones", async ({ page }) => {
    ...
    const degreeItems = page.locator("#education .font-semibold");
    const count = await degreeItems.count();
    expect(count).toBeGreaterThan(0);
  });

  // Skills
  test("Skills section shows strengths list with icons and descriptions", async ({
    page,
  }) => {
  ...
  // Certifications
  test("Certifications section loads cert tiles with links", async ({
    page,
  }) => {
  ...
  // Projects
  test("Projects section has carousel of project cards with previews", async ({
    page,
  }) => {
  ...
  //Reviews
  test("each review card should have name, review, LinkedIn link, and star ratings", async ({
    page,
  }) => {
    ...
    const firstCard = page
      .locator("section#reviews div.rounded-xl.shadow-lg")
      .first();
  ...
  // Contact
  test("Contact section displays email, LinkedIn and GitHub links", async ({
    page,
  }) => {
  ...
  // Download Resume
  test("Download resume button triggers file download", async ({
    page,
    context,
  }) => {
